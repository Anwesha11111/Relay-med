import { HeartPulse, ShieldCheck, Brain, Activity } from "lucide-react";
...
export function LoginPage() {
  ...
  const handleEmail = () => {
    if (name.trim() && email.trim()) {
      login("email", name.trim(), email.trim());
    }
  };
    ...
    <div className="min-h-screen flex" style={{ background: "var(--gradient-bg)", backgroundAttachment: "fixed" }}>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="soft-card p-8 w-full max-w-md fade-in">
          ...
          <p className="text-sm text-muted-foreground mt-1 mb-6">Sign in to access your personalized health dashboard</p>

          {/* OAuth Buttons */}
          <div className="space-y-3">
            <button
              onClick={() => login("google")}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border bg-white hover:bg-gray-50 transition-colors text-sm font-medium"
            >
            ...
            {!showEmail ? (
              <button
                ...
                className="w-full px-4 py-3 rounded-xl border bg-white hover:bg-gray-50 transition-colors text-sm font-medium"
              >
                Continue with Email
              </button>
            ) : (
              <div className="space-y-2">
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none focus:ring-2 ring-sage/30"
                />
                <input
                  value={email}
                ...
                <button
                  ...
                  className="w-full px-4 py-3 rounded-xl gradient-sage text-white text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50"
                >
                  Sign In
                </button>
              </div>
