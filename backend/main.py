"""
SecureMed AI Health Companion — FastAPI Application Entry Point
...
from backend.api.v1.routes import ingest, consent, reports, conversation, audit
...
async def startup_event():
    audit_logger.log(
        ...
        {"llm_provider": settings.LLM_PROVIDER, "version": "1.0.0"},
    )
    print(f"{settings.APP_NAME} started -- LLM: {settings.LLM_PROVIDER.upper()}")

# ── Health Check ──────────────────────────────────────────────────────────────
@app.get("/health", tags=["System"])
async def health_check():
    return {"status": "healthy", "app": settings.APP_NAME, "llm_provider": settings.LLM_PROVIDER}
